  ...
  while (ps5.isConnected() == true) {
    ...
    switch (mode) {
      ...
      case (true):
        ...
        // Ajuste de altura
        if (ps5.R2()) {
          Y_1_target = min(Y_1_target + mapf(ps5.R2Value(), 0, 255, 0, 1), max_altura);
          Y_2_target = min(Y_2_target + mapf(ps5.R2Value(), 0, 255, 0, 1), max_altura);
          Y_3_target = min(Y_3_target + mapf(ps5.R2Value(), 0, 255, 0, 1), max_altura);
          Y_4_target = min(Y_4_target + mapf(ps5.R2Value(), 0, 255, 0, 1), max_altura);
        }
        if (ps5.L2()) {
          Y_1_target = max(Y_1_target - mapf(ps5.L2Value(), 0, 255, 0, 1), min_altura);
          Y_2_target = max(Y_2_target - mapf(ps5.L2Value(), 0, 255, 0, 1), min_altura);
          Y_3_target = max(Y_3_target - mapf(ps5.L2Value(), 0, 255, 0, 1), min_altura);
          Y_4_target = max(Y_4_target - mapf(ps5.L2Value(), 0, 255, 0, 1), min_altura);
        }
        ...
        if (ps5.LStickY() > 100) {
          ii = 1;
          for (ii = 1; ii <= 100; ii++) {
            X_1_target = X_1_target + 0.4;
            X_2_target = X_2_target - 0.4;
            X_3_target = X_3_target + 0.4;
            X_4_target = X_4_target - 0.4;


            if (ii <= 100 / 2) {

              Y_1_target = Y_1_target - 0.35;
              Y_3_target = Y_3_target - 0.35;
            }
            if (ii > 100 / 2) {
          ...
          for (ii = 100; ii <= 200; ii++) {
            ...
            if (ii <= 150) {
              ...
              Y_2_target = Y_2_target - 0.35;
              Y_4_target = Y_4_target - 0.35;
            }
            if (ii > 150) {
        ...
        if (balance == true) {
          ...
          float var_1 = 1;
          if (acc_x > vari) {
            Y_1_target = min(Y_1_target + mapf(acc_x, 0, 5, 0, var_1), max_altura);
            Y_2_target = min(Y_2_target + mapf(acc_x, 0, 5, 0, var_1), max_altura);
            Y_3_target = max(Y_3_target - mapf(acc_x, 0, 5, 0, var_1), min_altura);
            Y_4_target = max(Y_4_target - mapf(acc_x, 0, 5, 0, var_1), min_altura);
          }
          if (acc_x < -vari) {
            Y_1_target = max(Y_1_target - mapf(acc_x, -5, 0, var_1, 0), min_altura);
            Y_2_target = max(Y_2_target - mapf(acc_x, -5, 0, var_1, 0), min_altura);
            Y_3_target = min(Y_3_target + mapf(acc_x, -5, 0, var_1, 0), max_altura);
            Y_4_target = min(Y_4_target + mapf(acc_x, -5, 0, var_1, 0), max_altura);
          }
          if (acc_y > vari) {
            Y_1_target = min(Y_1_target + mapf(acc_y, 0, 5, 0, var_1), max_altura);
            Y_2_target = max(Y_2_target - mapf(acc_y, 0, 5, 0, var_1), min_altura);
            Y_3_target = max(Y_3_target - mapf(acc_y, 0, 5, 0, var_1), min_altura);
            Y_4_target = min(Y_4_target + mapf(acc_y, 0, 5, 0, var_1), max_altura);
          }
          if (acc_y < -vari) {
            Y_1_target = max(Y_1_target - mapf(acc_y, -5, 0, var_1, 0), min_altura);
            Y_2_target = min(Y_2_target + mapf(acc_y, -5, 0, var_1, 0), max_altura);
            Y_3_target = min(Y_3_target + mapf(acc_y, -5, 0, var_1, 0), max_altura);
            Y_4_target = max(Y_4_target - mapf(acc_y, -5, 0, var_1, 0), min_altura);
          }
        }
...
void UpdateCoords() {
  ...
  //  X_1 = X_1 + mapf(X_1_target - X_1,-40,40,-5,5);
  //  X_2 = X_2 + mapf(X_2_target - X_2,-40,40,-5,5);
  //  X_3 = X_3 + mapf(X_3_target - X_3,-40,40,-5,5);
  //  X_4 = X_4 + mapf(X_4_target - X_4,-40,40,-5,5);

  //  Y_1 = Y_1 + mapf(Y_1_target - Y_1,-40,40,-5,5);
  //  Y_2 = Y_2 + mapf(Y_2_target - Y_2,-40,40,-5,5);
  //  Y_3 = Y_3 + mapf(Y_3_target - Y_3,-40,40,-5,5);
  //  Y_4 = Y_4 + mapf(Y_4_target - Y_4,-40,40,-5,5);
  ...
  X_1 = X_1 + scale * (X_1_target - X_1);
  X_2 = X_2 + scale * (X_2_target - X_2);
  X_3 = X_3 + scale * (X_3_target - X_3);
  X_4 = X_4 + scale * (X_4_target - X_4);

  Y_1 = Y_1 + scale * (Y_1_target - Y_1);
  Y_2 = Y_2 + scale * (Y_2_target - Y_2);
  Y_3 = Y_3 + scale * (Y_3_target - Y_3);
  Y_4 = Y_4 + scale * (Y_4_target - Y_4);
}
